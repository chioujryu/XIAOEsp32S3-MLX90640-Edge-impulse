...
uint8_t * jpg_buf_esp                = NULL; // points to the jpg output of the capture
uint8_t * jpg_buf_thermal            = NULL; // points to the jpg output of the capture
...
void loop()
{
  ...
  jpg_buf_esp                = (uint8_t *) ps_malloc(320*240*3*sizeof(uint8_t));
  jpg_buf_thermal            = (uint8_t *) ps_malloc(320*240*3*sizeof(uint8_t));
  ...
  if (takeNewPhoto) 
  {
    captureESP32PhotoSaveSpiffs(rgb888_buf_esp_320x240, rgb888_buf_esp_640x480);
...
void captureESP32PhotoSaveSpiffs(uint8_t * rgb888_buf_640x480, uint8_t * rgb888_buf_320x240) {
  ...
  fmt2jpg(rgb888_buf_esp_640x480, 640*480*3, 640, 480, PIXFORMAT_RGB888, 31, &jpg_buf_esp, &jpg_size);

...
// RAW DATA OF IR CAM
float mlx90640To[32*24]; // 用來存儲從 MLX90640 熱像儀中讀取的 768 個數據點。
...
void setup()
{
  ...
  Wire.begin();
  // Wire.setClock(400000) 設置 I2C 時鐘速率為 400 kHz，這是 I2C 協議的標準速率之一，通常用於高速數據傳輸。
...
void captureThermalPhotoSaveSpiffs( void ) {
  ...
  do 
  {
    ...
    Serial.print("\nmlx90640To = ");
    for (int i = 0 ; i < sizeof(mlx90640To)/ sizeof(float); i++){  
    ...
    Serial.printf("\nthermal_photo_save_to_SD_card     minTemp = %f, maxTemp = %f\n", minTemp, maxTemp);
  
    thermal_to_rgb888(); // 將開頭建立的 rgb888_buf_1 填滿 rgb 像素
...
size_t RGB888toJPG(uint8_t * rgb888_buf,uint16_t height,uint16_t width){
  ...
  free(rgb888_buf);  // 釋放照片緩衝區
  rgb888_buf = NULL;  // 釋放照片緩衝區
...
void esp32_photo_save_to_SD_card(const char * fileName) {
  ...
  Serial.println("ESP32 photo saved to file");
}

/*
// =======Save esp32 photo buffer to SD card=======
void esp32_photo_buffer_save_to_SD_card(const char * fileName) {
  bool ok = 0; // Boolean indicating if the picture has been taken correctly
  ...
  // 將 unit8_t 陣列 轉換成 string 陣列
  String rgb888_buf = "";
  for (int i = 0; i < 320*240*3; i++) {   // 768 = 24*32
    rgb888_buf += String(rgb888_buf_1[i]);
    if (i < 320*240*3 - 1) {
      rgb888_buf += ",";
    }
  }
  Serial.print("\n");
  Serial.println(rgb888_buf);

  const char * data = rgb888_buf.c_str();
  ...
  Serial.println("ESP32 photo buffer saved to file");
}
*/
...
void thermal_buffer_save_to_SD_card(const char * fileName){
  ...
  for (int i = 0; i < 768; i++) {   // 768 = 24*32
    thermalData += String(mlx90640To[i]);
    if (i < 767) {
      thermalData += ",";
    }
  }
...
void configInitCamera(){
  ...
  //config.pixel_format = PIXFORMAT_GRAYSCALE;  // Change to grayscale
  //config.grab_mode = CAMERA_GRAB_WHEN_EMPTY;
  config.fb_location = CAMERA_FB_IN_PSRAM;
  //config.jpeg_quality = 20;   // 越低相片品質愈好

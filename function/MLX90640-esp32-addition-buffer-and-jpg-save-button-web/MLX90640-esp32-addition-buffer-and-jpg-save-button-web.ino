...
void setup()
{
  ...
  // PinMode setting
  pinMode(CAPTURE_BUTTON, INPUT);       // sets the digital pin D1 as INPUT
  ...
  server.on("/saved_thermal_photo", []() {getSpiffImg(server, SPIFFS_FILE_PHOTO_MLX90640 , "image/jpg");});
...
void loop()
{
  ...
  digitalWrite(LED_BUILTIN, LOW);
  int val = digitalRead(CAPTURE_BUTTON);
  if (val != 1){savePhotoSD = true;}
